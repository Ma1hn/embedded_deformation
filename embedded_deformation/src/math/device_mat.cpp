...
#include "math/eigen_device_transfer.h"
#include "math/vector_ops.hpp"



mat33::mat33(const Eigen::Matrix3f &rhs) {
...
mat33& mat33::operator=(const Eigen::Matrix3f &rhs) {
	cols[0] = make_float3(rhs(0, 0), rhs(1, 0), rhs(2, 0));
	cols[1] = make_float3(rhs(0, 1), rhs(1, 1), rhs(2, 1));
	cols[2] = make_float3(rhs(0, 2), rhs(1, 2), rhs(2, 2));
	return *this;
...
mat34::mat34(const Isometry3f &se3) : rot(se3.linear().matrix()) {
	Eigen::Vector3f translation = se3.translation();
	trans = fromEigen(translation);
}

mat34::mat34(const Matrix4f &rhs) : rot(rhs.block<3, 3>(0, 0)) {
	Eigen::Vector3f eigen_trans = rhs.block<3, 1>(0, 3);
	trans = fromEigen(eigen_trans);
}

mat34::mat34(const ::float3 &twist_rot, const ::float3 &twist_trans) {
